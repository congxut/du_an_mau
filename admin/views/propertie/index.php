      ...
      <table class="table mt-3">
        ...
        <tbody>
          ...
          <tr>
            ...
            <?php }else{ ?>
            <td>
              <div style="height:30px;width:30px;border-radius:50px;background-color:<?=  $item["ma_color"] ?> ">
              </div>
            </td>

            <?php }?>
            <td>
              <a href="index.php?url=propertie-edit&id=<?=$item["kt_id"] ?>" class="btn btn-warning">Sửa</a>
            </td>
            <td>
              <a onclick="return confirm('Bạn có muốn xoá danh mục này ?')"
                href="index.php?url=propertie-delete&id=<?=$item["kt_id"] ?>" class="btn btn-danger">Xóa</a>
            </td>
          </tr>
          <?php  } ?>
